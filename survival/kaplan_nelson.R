...
# SOS => main formula to use: `surv` output is KM distr, `exp(-cumhaz)` is Nelson distr
...
#testthat::expect_equal(model$surv, exp(-model$cumhaz))
times = model$time
...
# `stype = 2` means S(t) = exp(-H(t)) and H(t) we get from Nelson-Aalen (`ctype = 1`)
